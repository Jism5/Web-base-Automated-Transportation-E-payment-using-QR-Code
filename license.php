...
if (isset($_POST['submit']) && isset($_FILES['my_image'])) {
	...
	if ($error === 0) {
		if ($img_size > 405000) {
		    ...
		    header("Location: Rsetting.php?error=$em");
		}else {
			...
			if (in_array($img_ex_lc, $allowed_exs)) {
				...
				$img_upload_path2 = 'adminfile/uploads/'.$new_img_name;
				copy($tmp_name, $img_upload_path2);
				move_uploaded_file($tmp_name, $img_upload_path);
				
				
				// Insert into Database
				$sql = "UPDATE rider_info
				        SET license_url = ('$new_img_name') , verify_status = 'Pending'
